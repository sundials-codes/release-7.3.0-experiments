...
int main(int argc, char *argv[]) {
  ...
  if (order < 3 || order > 5) {
    ...
    if (check_retval(&retval, "ARKodeSetOrder", 1)) {
      return 1;
    }
  } else {
    switch (order) {
  ...
  if (check_retval(&retval, "ARKodeSetMaxNumSteps", 1)) {
    return 1;
  }
  ...
  retval = SUNAdjointCheckpointScheme_Create_Fixed(
      SUNDATAIOMODE_INMEM, mem_helper, check_interval, ncheck, keep_check,
  ...
  if (check_retval(&retval, "ARKodeSetFixedStep", 1)) {
    return 1;
  }

  retval = ARKodeSetUseCompensatedSums(arkode_mem, 1);
  ...
  if (check_retval(&retval, "ARKodePrintAllStats", 1)) {
    return 1;
  }
  printf("\n");
  ...
  retval = ARKStepCreateAdjointStepper(arkode_mem, adj_rhs, NULL, tf, sf, sunctx,
                                       &adj_stepper);
  if (check_retval(&retval, "ARKStepCreateAdjointStepper", 1)) {
    return 1;
  }
  ...
  if (check_retval(&retval, "SUNAdjointStepper_Evolve", 1)) {
    return 1;
  }
  ...
  if (check_retval(&retval, "SUNAdjointStepper_PrintAllStats", 1)) {
    return 1;
  }
  printf("\n");

  printf("L2 Norm of u, dg/du0, dg/dp: %.16e, %.16e, %.16e\n",
         SUNRsqrt(N_VDotProd(u, u)), SUNRsqrt(N_VDotProd(sensu, sensu)),
         SUNRsqrt(N_VDotProd(sensp, sensp)));
